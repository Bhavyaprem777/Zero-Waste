...
const AdminDashboard = () => {
  ...
  const handleFlagDonor = (index) => {
    const updatedDonors = [...donors];
    updatedDonors[index].isFraudulent = true;
    localStorage.setItem('donationDataList', JSON.stringify(updatedDonors));
    setDonors(updatedDonors);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-100 py-10 px-4 sm:px-10">
      ...
      {!selectedDonor && !selectedReceiver && (
        <div className="grid md:grid-cols-2 gap-8">
          <div
            className="bg-white shadow-lg rounded-xl p-6 border border-blue-200 w-full overflow-x-auto"
       ...
       <div className="mt-8 flex justify-center">
              <button
                className="w-fit px-4 py-4 bg-green-600 text-white rounded-md hover:bg-green-700 text-sm whitespace-nowrap shadow-md"
                onClick={() => navigate('/analysis')}
              >
                📊 Food Analysis
              </button>
              {/* 👥 Onboard Volunteers/NGO Button */}
        {/* 👥 Onboard Volunteers/NGO Button */}
        {/* 👥 Onboard Volunteers/NGO Button */}
<button
  className="w-fit px-4 py-4 bg-blue-600 text-white rounded-md hover:bg-blue-700 text-sm whitespace-nowrap shadow-md ml-4"
...
>
  👥 Onboard Volunteers/NGO
      ...
      {selectedDonor && (
        <div className="grid md:grid-cols-2 gap-8">
          <div className="bg-white shadow-lg rounded-xl p-6 border border-blue-200 w-[800px]">
            <h2 className="text-2xl font-semibold mb-4 text-blue-700">🧑‍🍳 Donor Submissions</h2>
            {donors.length === 0 ? (
  ...
  <div className="flex flex-col gap-1">
    ...
    {!donation.isFraudulent ? (
      <button
        className="text-red-600 underline"
        onClick={() => handleFlagDonor(idx)}
      >
        🚩 Flag as Fraudulent
      </button>
    ) : (
      <span className="text-red-500 font-semibold">🚩 Fraudulent</span>
    )}
  </div>
</td>
          ...
          {selectedDonorIndex !== null && donors[selectedDonorIndex] && (
            <div className="col-span-2 mt-6 bg-white p-6 rounded-lg shadow-md border border-blue-300 w-full">
      ...
      {selectedReceiver && (
        <div className="grid md:grid-cols-2 gap-8">
          <div className="bg-white shadow-lg rounded-xl p-6 border border-green-200 w-[800px]">
            <h2 className="text-2xl font-semibold mb-4 text-green-700">📥 Receiver Requests</h2>
            {receivers.length === 0 ? (
